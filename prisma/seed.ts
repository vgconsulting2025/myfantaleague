...
type SeedPlayer = { name: string; role: "P" | "D" | "C" | "A"; club: string; quota: number; fm: number };
type SeedTeam = {
  slug: string;
  name: string;
  president: string;
...
async function main() {
  ...
  // Giornata iniziale (contesto per la prima edizione della Gazzetta).
  ...
  await prisma.flashNews.create({
    data: {
      text: "Mercato aperto: l'Agente è già al lavoro sulle rose della Lega Bar Centrale.",
      kind: "generic",
    },
  });
